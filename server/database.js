...
if (prodCount.c === 0) {
  ...
  const stmt = db.prepare(`INSERT INTO products (name, description, price, old_price, image, category_id, stock, featured) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`);
...
Object.entries(defaultSettings).forEach(([k, v]) => upsert.run(k, v));
